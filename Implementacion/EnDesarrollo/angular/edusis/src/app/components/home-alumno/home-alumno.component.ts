...
export class HomeAlumnoComponent implements OnInit {
  ...
  getAlumno(id: string) {
    this.dataApiService.getAlumno(id).then((respuesta) => {
  ...
  async regsitroCurso() {
    ...
    this.codigoCurso = (<HTMLInputElement>(
      document.getElementById("InputCodigoCurso")
    )).value.toString();
    if (this.codigoCurso == "") {
      Swal.fire(
        "Upss",
    ...
    await this.dataApiService
      .buscarCursoPorCodigo(this.codigoCurso)
      .then((respuesta) => {
        if (respuesta != -1) {
    ...
    if (this.chek_codigo == -1) {
      Swal.fire("Upss", "No encontramos el curso que deseas", "warning");
  ...
  getAllCursos() {
    this.dataApiService
      .getCursosDeAlumno(this.dataApiService.getUsuario())
      .then((respuesta) => {
  ...
  confirmarRegistro() {
    this.dataApiService
      .agregarAlumnoACurso(
        ...
        this.curso.id.toString()
      )
      .then((respuesta) => {
        this.recargar();
        Swal.fire("Felicitaciones!", "Ya formas parte del curso!", "success");
        this.esregistro = false;
      })
      .catch(() => {
        Swal.fire("Upss", "Algo salio mal al registrarse al curso :(", "error");
        this.esregistro = false;
      });
  }
